...
import { getDateToday, saveData, transfert_fond } from "./Utils";
            ...
            users.forEach(user => {
                if(user.number === sender.number) {
                    if(user.balance - amount >= 0) {
                        ...
                        transfert_fond(sender.number, receiver.number, amount, "debit", `Fund transfer to ${receiver.fullname} #${receiver.number}`);
                        setSender(user);
                        senderSuccess = true;
            ...
            if(senderSuccess) {
                users.forEach(user => {
                    if(user.number === receiver.number) {
                        ...
                        const transDate = new Date();
                        user.transactions.unshift({
                            title: `Fund transfer from ${sender.fullname} #${sender.number}`, 
                            amount: amount, 
                            type: "credit", 
                            date: getDateToday()
                        });
    ...
    return (
        <section id="main-content">
            <form id="form" onSubmit={transferFund}>
                ...
                <input type="text" name="amount" value={transferAmount} onChange={onTransfer} autoComplete="off" className="right big-input" />
