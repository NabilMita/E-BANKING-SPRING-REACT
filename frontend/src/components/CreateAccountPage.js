...
export const CreateAccountPage = (props) => {
    ...
    const createAcc = async (n,f,e,p,b,t) => {
        const options = {
            method: 'POST',
            url: 'http://localhost:8080/compte/addCompte',
             headers: {
    ...
    const createNewAccount = (user) => {
        ...
        createAcc(user.number, user.fullname, user.email, user.password, user.balance, user.type);
    ...
    return (
        <section id="main-content">
            <form id="form" onSubmit={handleCreateAccount}>
                ...
                <input id="balance" type="text" value={initialBalance} onChange={onInitialBalance} name="initialBalance" className="right" />
